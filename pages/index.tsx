...
function Home({announcements, dataCards}) {
  ...
  const isPWAInstalled = async () => {
    if ("getInstalledRelatedApps" in window.navigator) {
      ...
      relatedApps.forEach((app) => {
        ...
        if (
          app.url === "https://young-men.sycamoresstake.com/manifest.json" || app.url === "http://192.168.1.65:3000/manifest.json"
        ) {
          installed = true;
        }
      });
  ...
  return (
    <Layout>
      <Head>
        <title>Sycamores Stake Young Men</title>
      </Head>
      ...
      <div className="relative mb-3">
          <SectionHeader title="Announcements" subtitle="Stay up to date on upcoming YM events" className="pt-10"/>
      </div>
